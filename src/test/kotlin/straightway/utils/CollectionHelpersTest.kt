...
class CollectionHelpersTest {
    ...
    fun `repeatAsPattern starts with given sequence`() =
            Given {
                ...
                expect(it.result.take(size) is_ Equal to_ this)
            }
    ...
    fun `repeatAsPattern repeats the given sequence`() =
            Given {
                ...
                expect(it.result.take(2 * size).drop(size).take(size) is_ Equal to_ this)
            }
    ...
    fun `repeatAsPattern iterator next() throws if pattern is empty`() =
            Given {
                ...
                expect({ it.result.iterator().next() } does Throw.type<NoSuchElementException>())
            }
}